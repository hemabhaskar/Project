...
cor_matrix <- cor(temp[, c("incidenceRate", "PctWhite", "PctBlack", "PctAsian", "PctOtherRace")], method = "spearman")
...
#The country with the lowest mean of incidence rate is Arizona (361.78) and the country with the highest mean of incidence rate is Kentucky (509.55)
par(las = 1)
...
## Clustering counties by deathrate and incidence rate and see if it fits ----
cancer_d <- cancer[c(6,7,33)]
str(cancer)
str(cancer_d)
cancer_d
cancer_d.labels = cancer_d$Region

## We create a unlabeled data
cancer_nr <- cancer_d[1:(ncol(cancer_d)-1)]
str(cancer_nr)
table(cancer_d.labels)


## We scale the data
cancer_nr_sc <- scale(cancer_nr)
cancer_nr <- dist(cancer_nr_sc)

## We calculate the k means (4 clusters because there are four regions)
km.death <- kmeans(cancer_nr_sc, centers = 4)
km.death

km.reg <- km.death$cluster

rownames(cancer_nr_sc) <- cancer$Region
cancer_nr_sc

fviz_cluster(list(data=cancer_nr_sc, cluster=km.reg))

fviz_cluster(cancer_nr_sc)

table_cluster <- table(km.reg, cancer$Region)
...
#Add rectangular component for each cluster
rect.hclust(fit , k = 6, border = 1:6)
